...
-- A finding's subject and whether it drains (#396, deferred out of #374 / ADR-0019).
--
...
-- act on until it reaches zero is a defect; one permanently true by design is a gauge. The
-- CHECK makes the two-value vocabulary the schema's, not a convention.
--
-- Existing rows are backfilled rather than left behind: every subject is recoverable from the
-- details payload the detector already wrote, so the log stays queryable as provenance across
-- the boundary (the alternative -- starting the contract now -- would leave 4.8k rows carrying
-- a sentinel that means "unknown" and "dataset-wide" at once).
...
SET
    subject = CASE finding_type
        ...
        WHEN 'seat_multi_subscription'
            THEN format('%s|%s', details ->> 'user_email', details ->> 'as_of_date')
        WHEN 'seat_telemetry_after_close' THEN details ->> 'user_email'
        WHEN 'seat_emitter_without_seat' THEN details ->> 'user_email'
...
-- dq_finding_current gains the two dates the subject makes askable (#396). first_detected_at is
-- all-time -- "how long has this been known"; standing_since is the start of the unbroken run of
-- cycles ending at the current one -- "how long has it been standing without clearing". They
-- diverge exactly when a condition cleared and returned, which a single MIN(detected_at) would
-- misreport as one long run. Still a plain view: it reads a few thousand rows per query today
-- and a matview would go stale between cycles, the one failure mode the view exists to prevent.
--
-- Bound: cycles are enumerated from the log's own rows, so a cycle in which *nothing at all* was
-- detected leaves no trace and standing_since cannot see it as a gap. Recording every cycle
-- would need a spine the table does not have -- mart_refresh_log deliberately uses
-- clock_timestamp(), so its rows carry no cycle key to join detections onto. Accepted rather
-- than built: every live environment has a standing gauge true on every cycle, which makes each
-- cycle observable, and the case this misses is one where the whole log was momentarily clean.

DROP VIEW IF EXISTS marts.dq_finding_current;

CREATE VIEW marts.dq_finding_current AS
WITH cycle AS (
    -- refresh_all() has no COMMIT, so detected_at is transaction time and identifies a cycle
    -- exactly (ADR-0019). Ranking the distinct cycles turns "detected in consecutive cycles"
    -- into consecutive integers, which is what the streak below counts on -- the hourly
    -- schedule is not assumed, and a skipped or manual refresh does not read as a gap.
    SELECT
        c.detected_at,
        ROW_NUMBER() OVER (ORDER BY c.detected_at) AS cycle_seq
    FROM (SELECT DISTINCT detected_at FROM marts.dq_finding) AS c
),

detection AS (
    -- DISTINCT because one subject per cycle holds by construction, not by constraint: a
    -- duplicate would otherwise skew the island arithmetic silently instead of loudly.
    SELECT DISTINCT
        f.finding_type,
        f.subject,
        c.cycle_seq
    FROM marts.dq_finding AS f
    INNER JOIN cycle AS c ON f.detected_at = c.detected_at
),

island AS (
    -- Gaps and islands: cycle_seq minus its per-subject row number is constant across an
    -- unbroken run of cycles and shifts at every gap.
    SELECT
        d.finding_type,
        d.subject,
        d.cycle_seq,
        d.cycle_seq - ROW_NUMBER() OVER (
            PARTITION BY d.finding_type, d.subject ORDER BY d.cycle_seq
        ) AS island_key
    FROM detection AS d
),

streak AS (
    -- The one island reaching the newest cycle: a condition that cleared and came back
    -- starts a fresh island, so standing_since restarts with it rather than reading as an
    -- unbroken run since the first detection.
    SELECT
        i.finding_type,
        i.subject,
        MIN(c.detected_at) AS standing_since
    FROM island AS i
    INNER JOIN cycle AS c ON i.cycle_seq = c.cycle_seq
    GROUP BY i.finding_type, i.subject, i.island_key
    HAVING MAX(i.cycle_seq) = (SELECT MAX(cycle.cycle_seq) FROM cycle)
),
...
INNER JOIN first_seen AS fs
    ON f.finding_type = fs.finding_type AND f.subject = fs.subject
INNER JOIN streak AS s
    ON f.finding_type = s.finding_type AND f.subject = s.subject
