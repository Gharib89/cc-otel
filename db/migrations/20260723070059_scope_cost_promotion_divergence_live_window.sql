...
-- Scope cost_promotion_divergence to the live window (>= 2026-07-17, first purely-live
-- day per ADR-0006). The backfilled POC claude_code.cost.usage counter is inflated ~4x
-- vs the promoted api_request cost (root-caused in #198: POC-era per-request counter
-- emission, not a mart defect; see #215). An all-time reconciliation therefore fires
-- every refresh over known-irreconcilable history. fact_api_usage sources cost from
-- api_request events and is accurate; this check now monitors ongoing promotion fidelity
-- on live data only. Both sides gain a >= 2026-07-17 filter; nothing else changes.
...
-- Restore the all-time (unscoped) cost_promotion_divergence reconciliation.
