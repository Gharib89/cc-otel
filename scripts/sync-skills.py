...
Source of truth is the user-level skills dir (`~/.claude/skills`). The project
copies are fully *derived*: this tool replaces each listed skill's directory
verbatim, then re-applies the one project-owned divergence we allow — the
model-invocation flag. The same flag is stripped in the user-dir source copy
too, so a `model_invokable` skill is invokable in the local interactive session
(where personal skills shadow the project copies), not just in the cloud
sandbox.
...
Rule of thumb: any skill the cloud-ship chain composes (ship ->
tdd/code-review/copilot-pr-review-loop) must be model-invokable, so mark it
