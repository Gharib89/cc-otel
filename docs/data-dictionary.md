...
|---|---:|---:|---|---|
| `claude_code.token.usage` | 277,980 | 59.5% | 2026-07-17 | 2026-08-07 |
| `claude_code.cost.usage` | 69,495 | 14.9% | 2026-07-17 | 2026-08-07 |
| `claude_code.active_time.total` | 66,694 | 14.3% | 2026-07-17 | 2026-08-07 |
| `claude_code.lines_of_code.count` | 22,966 | 4.9% | 2026-07-17 | 2026-08-07 |
| `claude_code.code_edit_tool.decision` | 11,699 | 2.5% | 2026-07-17 | 2026-08-07 |
| `claude_code.usage.utilization` | 7,402 | 1.6% | 2026-07-17 | 2026-08-07 |
| `claude_code.usage.reset_in_seconds` | 7,402 | 1.6% | 2026-07-17 | 2026-08-07 |
| `claude_code.session.count` | 1,877 | 0.4% | 2026-07-17 | 2026-08-07 |
| `claude_code.commit.count` | 1,178 | 0.3% | 2026-07-18 | 2026-08-07 |
| `claude_code.pull_request.count` | 213 | 0.0% | 2026-07-18 | 2026-08-07 |

### Promoted columns

| column | type | non-null % | unique % | distinct | description | useful for |
|---|---|---:|---:|---:|---|---|
| `ts` | timestamp with time zone | 100.0% | 21.2% | 98,920 | Metric data-point timestamp. | time grain |
| `metric_name` | text | 100.0% | 0.0% | 10 | OTel instrument name. | signal routing |
| `metric_type` | text | 100.0% | 0.0% | 2 | Instrument kind: gauge/sum/histogram. | temporality handling |
| `value` | double precision | 100.0% | 38.9% | 181,401 | Numeric data-point value (delta counters). | all measures |
...
| `user_email` | text | 99.8% | 0.0% | 37 | Developer identity (normalized lowercase/trim). | dim_user join |
| `user_account_id` | text | 99.8% | 0.0% | 37 | user.account_id: Anthropic tagged account id. / user.account_uuid: Anthropic account UUID. |  |
| `organization_id` | text | 96.7% | 0.0% | 3 | Organization UUID. |  |
| `session_id` | uuid | 100.0% | 0.4% | 2,032 | Claude Code session UUID. | session facts |
| `model` | text | 79.3% | 0.0% | 7 | Model id (usage/cost/LOC metrics). | dim_model |
| `type_label` | text | 78.7% | 0.0% | 8 | claude_code.active_time.total: Active-time type: user (keyboard) / cli (tools+AI). / claude_code.lines_of_code.count: LOC change type: added / removed. / claude_code.token.usage: Token type: input/output/cacheRead/cacheCreation. | claude_code.active_time.total: active time split / claude_code.lines_of_code.count: loc measures / claude_code.token.usage: token breakdown |
| `tool_name` | text | 2.5% | 0.0% | 2 | Edit/Write/NotebookEdit. | fact_edit_decision |
| `decision` | text | 2.5% | 0.0% | 2 | accept / reject. | acceptance rate |
| `source` | text | 2.5% | 0.0% | 3 | Decision source: config/hook/user_*. | auto-vs-human split |
| `language` | text | 2.5% | 0.2% | 18 | Detected language. | language mix |
| `usage_window` | text | 3.2% | 0.0% | 2 | Rate-limit window (5h/7d/...). | fact_usage_window |
| `cc_version` | text | 96.8% | 0.0% | 26 | Claude Code version. | install health |
| `query_source` | text | 74.4% | 0.0% | 3 | Request origin: main/subagent/auxiliary. |  |
| `effort` | text | 71.9% | 0.0% | 5 | Reasoning-effort level. |  |
| `speed` | text | 0.0% | — | 0 | fast when fast-mode. |  |
| `agent_name` | text | 13.9% | 0.0% | 9 | Agent attribution (custom collapses). |  |
| `skill_name` | text | 27.3% | 0.1% | 85 | Skill attribution. |  |
| `plugin_name` | text | 8.9% | 0.0% | 4 | Plugin attribution. |  |
| `marketplace_name` | text | 6.5% | 0.0% | 1 | Marketplace attribution. |  |
...
| `scope_version` | text | 96.8% | 0.0% | 19 | Instrumentation scope version. |  |
...
| `terminal_type` | text | 88.0% | 0.0% | 8 | Terminal app type (terminal / VS Code / non-interactive). | surface split; non-interactive is adoption, not noise |
| `service_name` | text | 100.0% | 0.0% | 3 | Emitting surface: claude-code / claude-code-desktop / cowork. | desktop adoption; the only key separating the three surfaces |
| `os_type` | text | 96.8% | 0.0% | 2 | OS type; carries the WSL-vs-native-Windows split on its own. | fleet composition figure |
| `installer_stamp` | text | 0.2% | 0.1% | 1 | Installer stamp the emitting process started with: SHA256(wrapper + managed-settings + installer schema version). | fleet-config convergence: per seat, whether a re-push has reached the running process. A hash says same-or-different, never which of two is newer |
| `installer_stamp_on_disk` | text | 0.0% | 2.1% | 1 | Installer stamp the machine's managed-settings.json carried at emit time. | stale-session detection: a record whose installer_stamp differs from installer_stamp_on_disk is running a config the disk has already replaced |
...
| name | rows | % | first seen | last seen |
|---|---:|---:|---|---|
| `tool_decision` | 89,532 | 20.5% | 2026-07-17 | 2026-08-07 |
| `tool_result` | 89,192 | 20.5% | 2026-07-17 | 2026-08-07 |
| `api_request` | 83,539 | 19.2% | 2026-07-17 | 2026-08-07 |
| `hook_execution_complete` | 56,349 | 12.9% | 2026-07-17 | 2026-08-07 |
| `hook_execution_start` | 56,343 | 12.9% | 2026-07-17 | 2026-08-07 |
| `assistant_response` | 37,371 | 8.6% | 2026-07-17 | 2026-08-07 |
| `user_prompt` | 6,295 | 1.4% | 2026-07-17 | 2026-08-07 |
...
| `skill_activated` | 1,412 | 0.3% | 2026-07-17 | 2026-08-07 |
| `subagent_completed` | 1,262 | 0.3% | 2026-07-17 | 2026-08-07 |
| `api_error` | 920 | 0.2% | 2026-07-17 | 2026-08-05 |
| `at_mention` | 561 | 0.1% | 2026-07-17 | 2026-08-07 |
| `permission_mode_changed` | 307 | 0.1% | 2026-07-18 | 2026-08-06 |
| `internal_error` | 162 | 0.0% | 2026-07-18 | 2026-08-07 |
| `api_retries_exhausted` | 113 | 0.0% | 2026-07-17 | 2026-08-05 |
| `feedback_survey` | 112 | 0.0% | 2026-07-17 | 2026-08-07 |
| `compaction` | 102 | 0.0% | 2026-07-18 | 2026-08-07 |
| `auth` | 18 | 0.0% | 2026-07-20 | 2026-08-05 |
| `plugin_installed` | 9 | 0.0% | 2026-07-21 | 2026-08-05 |
| `api_refusal` | 5 | 0.0% | 2026-07-18 | 2026-08-06 |

### Promoted columns

| column | type | non-null % | unique % | distinct | description | useful for |
|---|---|---:|---:|---:|---|---|
| `event_time` | timestamp with time zone | 100.0% | 91.6% | 399,099 | Log-record timestamp. | time grain |
...
| `user_email` | text | 99.7% | 0.0% | 36 | Developer identity (normalized lowercase/trim). | dim_user join |
| `user_account_id` | text | 99.7% | 0.0% | 36 | user.account_id: Anthropic tagged account id. / user.account_uuid: Anthropic account UUID. |  |
| `organization_id` | text | 99.7% | 0.0% | 3 | Organization UUID. |  |
| `session_id` | uuid | 100.0% | 0.5% | 2,018 | Claude Code session UUID. | session facts |
| `prompt_id` | uuid | 96.7% | 1.5% | 6,319 | Prompt UUID. | prompt correlation |
| `model` | text | 28.3% | 0.0% | 7 | Model id (api_request/assistant_response). | fact_api_usage |
| `tool_name` | text | 41.0% | 0.0% | 34 | Tool name (tool_decision/tool_result; incl. mcp__*). | bridge_session_mcp |
| `duration_ms` | bigint | 40.9% | 20.7% | 36,888 | api_error: Failed request duration. / api_request: API request duration. / compaction: Compaction duration. / mcp_server_connection: MCP server connection duration. / subagent_completed: Subagent duration. / tool_result: Tool execution duration. |  |
| `input_tokens` | bigint | 19.2% | 2.0% | 1,703 | Prompt tokens. | fact_api_usage |
| `output_tokens` | bigint | 19.2% | 6.9% | 5,777 | Completion tokens. | fact_api_usage |
| `cache_creation_tokens` | bigint | 19.2% | 14.5% | 12,074 | Cache-write tokens. | fact_api_usage |
| `cache_read_tokens` | bigint | 19.2% | 81.1% | 67,785 | Cache-read tokens. | fact_api_usage |
| `cost_usd` | double precision | 19.2% | 96.4% | 80,554 | Estimated cost. |  |
| `cc_version` | text | 100.0% | 0.0% | 26 | Claude Code version. | install health |
| `event_sequence` | bigint | 100.0% | 2.9% | 12,501 | Per-session event ordinal. |  |
| `request_id` | text | 27.8% | 69.1% | 83,684 | Anthropic API request id. |  |
| `speed` | text | 19.4% | 0.0% | 1 | fast/normal. |  |
| `effort` | text | 18.5% | 0.0% | 5 | Reasoning-effort level. | fact_api_usage |
| `query_source` | text | 28.0% | 0.0% | 19 | Request origin. | fact_api_usage |
...
| `hook_event` | text | 27.0% | 0.0% | 10 | Hook trigger event. |  |
...
| `agent_name` | text | 4.3% | 0.0% | 9 | Agent attribution (api_request). | bridge_session_agent |
...
| `success_bool` | boolean | 20.6% | 0.0% | 2 | Success flag where reported. |  |
| `tool_use_id` | text | 41.0% | 50.1% | 89,543 | Tool invocation id. |  |
| `decision` | text | 20.5% | 0.0% | 2 | accept / reject (tool_decision). |  |
| `source` | text | 20.5% | 0.0% | 6 | Decision source (tool_decision). |  |
| `scope_name` | text | 100.0% | 0.0% | 1 | Instrumentation scope. |  |
| `scope_version` | text | 100.0% | 0.0% | 19 | Scope version. |  |
...
| `service_name` | text | 100.0% | 0.0% | 3 | Emitting surface: claude-code / claude-code-desktop / cowork. | desktop adoption; the only key separating the three surfaces |
| `os_type` | text | 100.0% | 0.0% | 2 | OS type; carries the WSL-vs-native-Windows split on its own. | fleet composition figure |
| `terminal_type` | text | 93.1% | 0.0% | 8 | Terminal app type (terminal / VS Code / non-interactive). | surface split; non-interactive is adoption, not noise |
...
| `subagent_total_tokens` | bigint | 0.3% | 99.1% | 1,251 | Tokens consumed by the subagent run. | query_source='agent:custom' collapses every custom agent into one bucket |
...
| `decision_source` | text | 12.7% | 0.0% | 3 | Who authorised the tool call (config/hook/user_temporary/...). | permission friction: user_temporary on 12 of 13 seats |
...
| `num_hooks` | smallint | 25.9% | 0.0% | 4 | Hooks matched for the event. | how much hook machinery the fleet runs |
| `num_success` | smallint | 12.9% | 0.0% | 5 | Hooks that succeeded. | with num_hooks, states the failure count exactly |
| `hook_source` | text | 27.0% | 0.0% | 5 | Where the hook came from (settings/pluginHook/...). | attributes hook load to plugins, which the plugin columns cannot |
| `total_duration_ms` | bigint | 12.9% | 10.5% | 5,894 | Total time across the hooks run for one event. | hook overhead: bridge_session_hook has executions and no time |
| `installer_stamp` | text | 0.3% | 0.1% | 1 | Installer stamp the emitting process started with: SHA256(wrapper + managed-settings + installer schema version). | fleet-config convergence: per seat, whether a re-push has reached the running process. A hash says same-or-different, never which of two is newer |
