...
uv run python -m tools.matview_sync --check    # gate: canonical db/views/ + db/functions/ files <-> live catalog converge, both ways, all kinds (needs Docker)
uv run python -m tools.matview_sync --name <slug>  # author: edited canonical file -> generated migration (matview DROP+CREATE; view/function up is CREATE OR REPLACE, no DROP; git-HEAD down body, DROP-prefixed when OR REPLACE cannot restore it -- probed against the up state, refuses rather than write a down that cannot run, #437); then dev-migrate.sh regenerates schema.sql
...
uv run python -m tools.front_door [--env interim|prod] [--days 14|--since|--until]  # read-only: requests per UTC day at an environment's front door, split by statusCode (ADR-0027, #431); exit 1 while STILL RECEIVING, 0 once 7 consecutive complete days carry zero 200s; needs az login + Monitoring Reader and AZURE_SUBSCRIPTION_ID + RESOURCE_GROUP from the target .env.<env>
...
- Interim env: VS-benefits subscription (`rg-cc-otel-interim`, swedencentral) — write-quiet since the ingest repoint (ADR-0021) and no longer the env the report reads (prod since 2026-08-03, #247/#248, ADR-0022); it stays deployable until its RG is deleted. The prod RG is granted and its server `ccotel-pg-prod` exists (since 2026-07-15); interim retires at the cutover gate (ADR-0004, #248 Part B). Bicep is dual-target.
...
- **Schema status watch**: `env-schema-status.yml` runs `dbmate status --exit-code` against prod on a daily cron (05:00 UTC) plus `workflow_dispatch`; a pending migration fails the run. Interim's matrix leg was dropped ahead of its retirement — permanently behind `main`, so a daily red run would mask prod's verdict (#439). Detection only — the fix is a deploy. It exists because a database behind `main` still answers every `SELECT *` partition, so a Power BI refresh reports success while dropping the missing columns from the model (ADR-0023, #414).
...
| `tools/` | Curation + ops tooling over the blob reservoir (sweep, basis drift, data dictionary, replay, scrub, compact) + reference-data ingest (`roster_load.py`) + the interim->prod cutover copy + terminal sweep (`cutover_copy.py`, ADR-0020/0021) + its reservoir half (`reservoir_copy.py`, #246) + the front-door silence measurement (`front_door.py`, ADR-0027) + CI gate-path derivation (`gate_paths.py`) |
...
- **Everything is a migration** — views, grants, matviews, column-registry rows all land via dbmate; CI checks schema drift; never edit the schema out-of-band. Canonical-definition bodies are the exception to hand-authoring: every view, matview, and function in `marts` + `staging` is edited in its canonical file (`db/views/<schema>/<slug>.sql` / `db/functions/<schema>/<slug>.sql`) and `matview_sync --name` generates the migration — never hand-paste an object body into a new `dbmate new` migration (#263, ADR-0026).
