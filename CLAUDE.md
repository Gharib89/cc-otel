...
uv run python -m tools.front_door [--env interim|prod] [--days 14|--since|--until]  # read-only: requests per UTC day at an environment's collector ingress, split by statusCode (ADR-0027, #431); exit 1 while STILL RECEIVING, 0 once 7 consecutive complete days carry zero 200s; needs az login + Monitoring Reader and AZURE_SUBSCRIPTION_ID + RESOURCE_GROUP from the target .env.<env>
