...
- **`.ps1` files must be pure ASCII.** The `ci-powershell` job throws on **any** PSScriptAnalyzer finding, Warnings included; `PSUseBOMForUnicodeEncodedFile` fires on a single non-ASCII byte (one em-dash `-` in a comment) while local `-Severity Error` stays green. Use ASCII; grep the diff: `LC_ALL=C grep -n '[^ -~\t]' file.ps1`.
